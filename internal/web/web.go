...
func New(cat *domain.Catalog) *Server {
	...
	s.mux.HandleFunc("/api/game/titles", s.needSave(s.handleTitles))
	s.mux.HandleFunc("/api/game/titles/unlock", s.needSave(s.handleSetTitleUnlocked))
	s.mux.HandleFunc("/api/game/titles/unlock-all", s.needSave(s.handleUnlockTitles))
